...
function computeUnrealizedAndOpenValue(
  positionsByKey: Map<string, PositionAccumulator>,
  markPriceByKey: Map<string, number>,
): { unrealizedPnl: number; openMarketValue: number } {
  let unrealizedPnl = 0;
  let openMarketValue = 0;

  for (const position of positionsByKey.values()) {
    ...
    const marketMark = markPriceByKey.get(position.key);
    const fallbackMark =
...
export function reduceTrackedWalletEvents(input: {
  ...
  for (const event of sortedEvents) {
    ...
    const { unrealizedPnl, openMarketValue } = computeUnrealizedAndOpenValue(
      state.positionsByKey,
      markPriceByKey,
    );
